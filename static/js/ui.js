...
const UI = (() => {
  ...
  function goStep(n) {
    if (n === 3 && !UrlBuilder.getUrl()) {
      showAlert('step2-alert', 'Sélectionnez d\'abord une colonne de recherche.', 'error');
      return;
    }
    currentStep = n;
    ...
    if (n === 3) {
      UrlBuilder.build();
      Config.renderSummary(currentRid, UrlBuilder.getSearchCol(), UrlBuilder.getMode(), Filters.getAll(), columns);
    }
  }
  ...
  async function loadDataset() {
    ...
    if (rid) {
      ...
      if (!slug) {
        showStatus(statusEl, 'URL non reconnue. Collez une URL data.gouv.fr ou un rid (UUID).', 'error');
        resetBtn(btn);
        return;
      }
  ...
  async function loadProfile(rid) {
    ...
    try {
      ...
      showStatus(statusEl, `${columns.length} colonnes chargées · rid : ${rid}`, 'success');
  ...
  function filterCols() {
    colFilter = document.getElementById('col-search')?.value.toLowerCase() || '';
    renderSearchCols(columns);
  }
  ...
  function renderSearchCols(cols) {
    ...
    if (!filtered.length) {
      el.innerHTML = '<p class="fr-text--sm fr-text--mention">Aucune colonne correspondante.</p>';
      return;
    }
  ...
  function _selectCol(el, col) {
    document.querySelectorAll('.col-item').forEach(x => x.classList.remove('selected'));
  ...
  function exportJson() {
    Config.exportJson(currentRid, UrlBuilder.getSearchCol(), UrlBuilder.getMode(), Filters.getAll(), UrlBuilder.getUrl());
  }

  function exportYaml() {
    Config.exportYaml(currentRid, UrlBuilder.getSearchCol(), UrlBuilder.getMode(), Filters.getAll(), UrlBuilder.getUrl());
  }
...
function loadDataset() { UI.loadDataset(); }
function goStep(n) { UI.goStep(n); }
function copyUrl() { UI.copyUrl(); }
function testUrl() { UI.testUrl(); }
function reset() { UI.reset(); }
function filterCols() { UI.filterCols(); }
function runTest() { UI.runTest(); }
